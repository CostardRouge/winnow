...
- 2026-09-02 — **Moving an indexed original to another folder deadlocks the pipeline**: the file is dropped as an unverifiable duplicate while its row is flagged missing + auto-trashed, and no rescan ever repairs it. Nothing detects a move today. `docs/memory/pipeline.md` carries the mechanism, the recovery (relink off the `duplicate_hits` map — the old→new mapping is already in the DB) and the two traps: purging does **not** release `content_hash` (unlike the dedup reclaim path), and "Keep only this copy" reclaims instead of relinking a trashed library copy. Fix of record: store `(st.dev, st.ino)` and treat an ENOENT-pathed collision as a move. Maintainer's call on sequencing.
