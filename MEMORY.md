...
- A body a file never named is voted on, never guessed: five weighted signals propose, a human applies, `device_source` + the indexer guard keep it → `docs/memory/pipeline.md`
...
- 2026-08-20 (revised 2026-09-20) — **Three duplicate migration prefixes are on `main`**, contradicting rule 1 of `db/migrations/README.md` and its "History" section, which reads as though every collision was resolved: `0010_gps_coords.sql` / `0010_search_text.sql`, `0013_asset_groups.sql` / `0013_clean_object_placeholders.sql`, and — new — `0042_device_attribution.sql` (#253, merged second) / `0042_geo_exempt.sql` (#254, merged first and already applied on the Optiplex). They apply today in an accidental lexicographic order. Per rule 1 the device one is the one to renumber, to the tail; renumbering an applied file means extending `RENUMBERED` in `src/lib/migrate.ts` and touches every already-migrated database — maintainer's call. **Next free number is `0043`.**
...
- **`docs/UNPLACED.md`** — the brief, now a **record** (shipped 2026-09-20),
  ...
  invariant Atelier's trip legs depend on, §7–§8 the screen and the six
  printed rules. Read it before touching `gps_source`, the geotag flow,
  `/api/assets/geo` or `/library/incoming/unplaced`.
