...
- 2026-09-19 — **A drone video belongs to no camera.** DJI writes no `Make`/`Model` in its MP4s, so every device-keyed view drops those clips — the `/gear` shelf, the `?device=` grids it links to, the viewer's Camera/Device rows — while the very same aircraft's *photos* group fine under `DJI FC8482`. `docs/memory/pipeline.md` carries the mechanism, what the file actually holds (a `Model` reachable only through exiftool's `-ee` timed-metadata track, at a real I/O cost) and the "one aircraft, two spellings" trap that any fix has to dodge. The decision is the maintainer's — read the embedded track, infer the body from the `.SRT` sidecar, or let a human assign one once — and the plumbing it needs is the same either way: a provenance column and a re-index guard, exactly like `gps_source='manual'`. Until one lands, the drone's clips are reachable by folder (`?under=`) or by a hand-applied tag, nothing else.
