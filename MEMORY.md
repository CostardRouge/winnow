...
- 2026-09-14 — **The Settings review is written and nothing in it is fixed**: `docs/SETTINGS-UI.md`, 26 findings, all *open*. Its §5 orders the work (presentation pass first, routes last) but **step 0 is a decision, not a commit**: D15 asks whether a settings pane should follow the runtime (live vs restart, today's rule) or the decider (operator / photographer / moment), and the answer is the shape of the rail every other finding assumes. Maintainer's call. **D16/E22/E26 are Fixed** — the geocoding rate + cell size now have a control group on Settings › Pipeline and the Live Photo companion a toggle in the Exports toolbar. Four gaps are features rather than settings work and each wants its own brief: quiet hours (E19, the one missing *concept* — pacing is a flat per-hour cap with no notion of a time window), a trash retention policy (E20), a configurable import filing template (E21 — hardcoded at `src/lib/import.ts:76`, and the one place Winnow creates structure in the originals' world), and any notification at all (E23 — there is none, of any kind).

- 2026-09-13 — **The UI review is a proposal; nothing from it has shipped.** `docs/UI-REVIEW.md` carries 32 numbered findings in three tiers: P0 is a day of bugs (a phone header collision, two wrapping toolbars, clipped Settings tabs, `--color-faint` below AA), P1 is the visible step (one `PageHeader`, the session card and page hierarchy, colour semantics, control heights), P2 the consolidation of five tab families, four chip families, 35 font sizes. The maintainer picks the tier and the order; until then the rules in `docs/memory/frontend.md` ("`docs/UI-REVIEW.md` is the design backlog") apply to any new UI so the drift stops growing.
