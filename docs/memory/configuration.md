...
**The environment tier is now visible, still not editable**: `/settings/instance` prints its effective values with, per row, whether they came from the environment or from the schema default — which is what makes the compose drift below findable instead of silent. Read `docs/memory/frontend.md` before touching that page; the short version is that it is a server component with no endpoint and that its `force-dynamic` stops the CI runner's environment being baked into the image.
