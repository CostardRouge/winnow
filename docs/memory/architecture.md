...
**How to apply**: the only path from an inference to a coordinate on an asset is an explicit action that goes through the existing geotag flow with its recap, on an explicit selection. A chapter's own location field (when it lands) writes on the chapter, then *proposes* to geotag the chapter's GPS-less media as a second, distinct gesture — never implicitly.
