...
**Decision**: `LoadingOverlay` (`src/app/ui.tsx`, class `.zone-loading-overlay` in `globals.css`) is `LoadingState`'s exact spinner + label, drawn as a card floated over its zone with `backdrop-filter: blur(3px)` — the modal treatment minus the dialog. `/timeline` renders it as its **only** loader: the first read draws it over an empty `.tl-body`, every later one (cut rule, place granularity, library source, a chapter edit, "Réessayer") over the previous answer, which stays visible, blurred and unclickable. Its label names the change being applied (`readingLabel()` diffs the previous `{mode, gran, source}` against the new one), not a generic "chargement".
