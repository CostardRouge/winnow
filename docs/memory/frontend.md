...
**The toggle wraps below 860px**: eight buttons overflow a phone, and `.view-toggle` does not scroll — Marks and Record simply left the screen with nothing to reveal they existed. `.gear-head .view-toggle` gets `flex-wrap` in the narrow block. Any ninth layout makes this worse; that is the point at which the picker should become something other than a segmented control.
