...
| `GET /api/capabilities` | What **this instance** can do, for a client app (Atelier): API version, auth methods + whether CORS is on, media facts (sidecars, Range on derivatives / not on originals, proxy codec & size, the partial `content_hash`), the document bucket (kinds + body cap), the unbuilt reminders flag, storage driver, and the caller's role. Facts about the code as it is — a client decides from these rather than guessing |
| `GET /api/apps/:app/docs` `?kind=trip\|project\|roll\|presets\|lutpack` | A client app's **own documents** (Atelier's road trips, later its projects): opaque JSON Winnow never reads, scoped to the signed-in user — any role, since a viewer owns its trips as it owns its password. Bodies inlined, newest first |
| `GET` · `PUT` · `DELETE /api/apps/:app/docs/:id` | One document, guarded by a revision: `GET` answers with an `ETag` (`If-None-Match` → 304); `PUT` `{ kind, version, doc }` (`application/json` required, 1 MiB cap → 413) needs `If-Match` on an existing row — stale or missing → **412** `{ etag, updated_at }` so the client decides, never a merge; `DELETE` with `If-Match` → 204. A row that is not the caller's answers **404**, never 403 |
| `GET /api/apps/:app/files` | The blobs this user keeps for the app: ids, sizes, what is used and what the caps are. Never bytes — it is what turns "push my whole pack" into "push the three you are missing" |
| `GET` · `PUT` · `DELETE /api/apps/:app/files/:sha256` | One blob, **content-addressed**: the id IS the SHA-256 of the bytes and the route refuses a body that does not hash to it (400). `GET` answers the bytes with that hash as `ETag` and `Cache-Control: private, immutable` (`If-None-Match` → 304); `PUT` takes a raw body (16 MiB cap → 413, 512 MiB per user and app → 507) and is a no-op when the hash is already here; `DELETE` → 204. Not the caller's → **404**. For what a document cannot hold: Atelier's purchased LUT lattices, 1.5-2 MB apiece |
...
and a session is a **directory on disk**, never a leg of a trip. The
**Timeline** is the third axis — a chronological stream cut into **chapters**
("Sydney, 3 → 11 March") that cross those folders, each listing the folders it
merges so the merge is visible, with a sticky per-month **spine** to reach any
month of a year in one gesture. It carries the Incoming/Gallery/All picker like
`/gear` and `/search`, tiles open the same viewer with the same P/X/0-5
shortcuts, and *Ouvrir dans la grille* drops the chapter's date span into the
grid.

Chapters are **derived on every request, never stored** (`lib/timeline.ts`):
one ordered SQL scan (`LAG` + a running sum of breaks) returns *one row per
run* — a 100k library collapses to a few hundred rows — then a pure JS pass
absorbs the crumbs, and one more query picks a cover and a spread sample per
chapter. Three cut rules share that scan:

- **Lieu** — a different place starts a chapter. Merges a nine-day stay across
  three folders; shatters a day on the road through six villages.
- **Temps** — a silence longer than `gap` hours starts a chapter. The road day
  becomes one chapter; the stay breaks every night.
- **Hybride** (default) — a place change always breaks; staying put only after
  72 h (sleeping there is not leaving); then runs under `absorb` media fold
  into their neighbour when they belong to the same outing.

The **place granularity** is picked automatically (Région → Département →
